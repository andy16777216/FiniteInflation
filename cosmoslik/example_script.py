...
class main(SlikPlugin):
    ...
    def __init__(self):
        ...
        self.cosmo = get_plugin('models.cosmology')(
            ...
            omnuh2 = 0.000645,
            massive_neutrinos= 3.046,#param( 3, .2),
            massless_neutrinos=0 #param(3,.2),
        )
	...
	#print 'loading likelihoods'
        ...
        self.get_cmb = get_plugin('models.pico')(
            datafile='../data/pico3_tailmonty_v34.dat'
        )
	...
	#print 'loading sampler'
        self.sampler = get_plugin('samplers.metropolis_hastings')(
             self,
             num_samples=1000000,
             output_file='chains/camspec_lowl.lcdm_neff_neffphase.chain',
             proposal_cov='../data/proposal.covmat',
             proposal_scale=1,
             print_level=0,
             output_extra_params=['cosmo.Yp','cosmo.H0', 'cosmo.neff_phase', 'cosmo.leq', 'phase_template.alpha']
        )
...
if __name__=='__main__':
     #run the chain
     for _ in Slik(main()).sample(): passfrom cosmoslik import param_shortcut, lsum, get_plugin, SlikDict, SlikPlugin, Slik
from numpy import interp, identity, exp, inf, arange, hstack, loadtxt, zeros, ones, log, invert
import sys
import math        
param = param_shortcut('start','scale')
...
class main(SlikPlugin):
	...
	#print 'loading derivers'
        self.bbn = get_plugin('models.bbn_consistency')()
        self.hubble_theta = get_pluginahhahi('models.hubble_theta')()
        self.priors = get_plugin('likelihoods.priors')(self)
