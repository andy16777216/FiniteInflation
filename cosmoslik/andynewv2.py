...
class main(SlikPlugin):
    ...
    def __init__(self):
        ...
        self.cosmo = get_plugin('models.cosmology')(
            logA = param(3.2),
            ns = param(0.96),
            k_c = param(-4, range = (-15,10), scale = 1),
            alpha_exp = param(0.8, range=(0.6,1), scale = 0.2),
            ombh2 = param(0.0221),
            omch2 = param(0.12),
	...
	#print 'loading sampler'
        self.sampler = get_plugin('samplers.metropolis_hastings')(
             self,
             num_samples=1000000,
             output_file='chains/andynewv1.chain',
             proposal_cov='../data/proposal.covmat',
             proposal_scale=1,
             #print_level=0,
             output_extra_params=['cosmo.Yp','cosmo.H0','cmb_result'] #,'cl_TT'
        )
