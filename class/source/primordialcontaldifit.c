/*modified with k_c is the log of the cutoff k  ANDY */
  ...
  if (ppm->lnk_size > 0) {
    ...
    if (ppm->primordial_spec_type == analytic_Pk) {
      for (index_md = 0; index_md < ppm->md_size; index_md++) {
        ...
        free(ppm->running[index_md]); /* ANDY */
        free(ppm->cutoff[index_md]);
        free(ppm->exponent[index_md]);
      }
      free(ppm->amplitude);
      free(ppm->tilt);
      free(ppm->running); /* ANDY */
      free(ppm->cutoff); 
      free(ppm->exponent); 
    }
    else if (ppm->primordial_spec_type == external_Pk) {
...
/* ANDY */
  ...
  class_alloc(ppm->cutoff,
              ppm->md_size*sizeof(double *),
              ppm->error_message); /* ANDY */

  class_alloc(ppm->exponent,
              ppm->md_size*sizeof(double *),
              ppm->error_message); /* ANDY */
  ...
  for (index_md = 0; index_md < ppm->md_size; index_md++) {
    ...
    class_alloc(ppm->running[index_md],
                ppm->ic_ic_size[index_md]*sizeof(double),
                ppm->error_message);
    class_alloc(ppm->cutoff[index_md],
                ppm->ic_ic_size[index_md]*sizeof(double),
                ppm->error_message); /* ANDY */
    class_alloc(ppm->exponent[index_md],
                ppm->ic_ic_size[index_md]*sizeof(double),
                ppm->error_message); /* ANDY */
  ...
  for (index_md = 0; index_md < ppm->md_size; index_md++) {
    ...
    for (index_ic1 = 0; index_ic1 < ppm->ic_size[index_md]; index_ic1++) {
	  ...
	  one_cutoff = ppm->k_c;
	  one_exponent = ppm->alpha_exp; /* ANDY */
        }
      ...
      ppm->cutoff[index_md][index_ic1_ic2] = one_cutoff; /* ANDY */
      ppm->exponent[index_md][index_ic1_ic2] = one_exponent; /* ANDY */
    }
    ...
    for (index_ic1 = 0; index_ic1 < ppm->ic_size[index_md]; index_ic1++) {
      for (index_ic2 = index_ic1+1; index_ic2 < ppm->ic_size[index_md]; index_ic2++) {
        ...
        if (one_correlation == 0.) {
          ...
          ppm->cutoff[index_md][index_ic1_ic2] = 0.;
          ppm->exponent[index_md][index_ic1_ic2] = 0.; /* ANDY */
        }
        else {
          ppm->is_non_zero[index_md][index_ic1_ic2] = _TRUE_;
          ...
          ppm->running[index_md][index_ic1_ic2] =
            ...
            + one_running;
          ppm->cutoff[index_md][index_ic1_ic2] =
            0.5*(ppm->cutoff[index_md][index_ic1_ic1]
                 +ppm->cutoff[index_md][index_ic2_ic2])
            + one_cutoff;
          ppm->exponent[index_md][index_ic1_ic2] =
            0.5*(ppm->exponent[index_md][index_ic1_ic1]
                 +ppm->exponent[index_md][index_ic2_ic2])
            + one_exponent; /* ANDY */
        }
      }
    }
...
int primordial_analytic_spectrum(
  ...
  if (ppm->is_non_zero[index_md][index_ic1_ic2] == _TRUE_) {
    *pk = ppm->amplitude[index_md][index_ic1_ic2]*(1.-exp(-pow((k/exp(ppm->cutoff[index_md][index_ic1_ic2])),1./(1.-ppm->exponent[index_md][index_ic1_ic2])-1.)))*exp((ppm->tilt[index_md][index_ic1_ic2]-1.)*log(k/ppm->k_pivot)+ 0.5 * ppm->running[index_md][index_ic1_ic2] * pow(log(k/ppm->k_pivot), 2.));
/* ANDY */
  }
  else {
    *pk = 0.;
